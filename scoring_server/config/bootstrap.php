...
/*
 * Initializes default Config store and loads the main config file (app.php)
 *
 * CakePHP contains 2 config files after project creation:
 * - `config/app.php` for the default application config.
 * - `config/app_local.php` for environment specific config.
 */
try {
...
/*
 * Load an environment local config file to provide overrides to your config.
 ...
 * Notice: For security reasons app_local.php **should not** be included in your git repo.
 *
 * app_local.php is only loaded if the host exists of a single name without domain or if the php
 * is run from a test.
 */
if (
  file_exists(CONFIG . 'app_local.php') &&
  (!isset($_SERVER['HTTP_HOST']) || !str_contains($_SERVER['HTTP_HOST'], '.'))
) {
...
/*
 * Apply the loaded config settings to their respective systems.
