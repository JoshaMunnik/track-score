...
export function IndividualConfigurationForm() {
  const {
    active,
    configuration,
    updateConfiguration
  } = useIndividualStore(useShallow((state) => ({
    active: state.active,
    configuration: state.configuration,
    updateConfiguration: state.updateConfiguration,
  })));
  const [showConfirm, setShowConfirm] = useState(false);
  const [newConfiguration, setNewConfiguration] = useState<Partial<IndividualConfigurationModel>>(
    {}
  );

  function checkAndUpdate(value: Partial<IndividualConfigurationModel>): void {
    if (active) {
      setNewConfiguration(value);
      setShowConfirm(true);
    } else {
      // this method is only called when the action would restart the game, so restart it always
      update(value, true);
    }
  }

  function update(value: Partial<IndividualConfigurationModel>, restart: boolean): void {
    updateConfiguration(value, restart, useIndividualConfigurationsStore);
  }
    ...
    <>
      <Paper>
        <Grid templateColumns="1fr auto" form>
          ...
          <GridInputItem>
            <NumberField
              value={configuration.minPlayerCount}
              min={1}
              max={configuration.maxPlayerCount}
              onChange={(value) => checkAndUpdate({minPlayerCount: value})}
            />
          </GridInputItem>
          <GridLabelItem>
          ...
          <GridInputItem>
            <NumberField
              value={configuration.maxPlayerCount}
              min={configuration.minPlayerCount}
              max={16}
              onChange={(value) => checkAndUpdate({maxPlayerCount: value})}
            />
          </GridInputItem>
          <GridLabelItem>
            Track dealer
      ...
      >
        Changing this setting will restart the game. Continue?
      </ConfirmPopup>
    </>
  );
}
